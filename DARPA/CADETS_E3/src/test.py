...
# to reconstruct the expected relation type for each edge. The reconstruction error (typically a 
# cross-entropy loss) measures how well the model predicts the true relation type.

# ANOMALY DETECTION:
# Each edge’s reconstruction loss acts as its anomaly score — a higher loss implies the model 
# found the interaction behavior unusual or unexpected. Losses are accumulated and averaged 
# within defined temporal windows (based on time_window_size), producing time-window-level 
# anomaly summaries and detailed per-edge rankings.
# ----------------------------------------------------------------------------------------------
